...
// ============================================================
// IMPL BLOCKS FOR LOOT EDITOR (no duplicate struct defs)
// ============================================================
...
impl LootRoller {
    ...
    pub fn roll_table(&mut self, table: &LootTable, _catalog: &[Item], ctx: &mut DropContext) -> Vec<DropResult> {
...
impl MonteCarloResult {
    ...
    pub fn std_dev_value(&self) -> f32 {
        ...
        let variance = self.total_value_per_run.iter().map(|&v| (v-mean).powi(2)).sum::<f32>() / (self.total_value_per_run.len()-1) as f32;
        variance.sqrt()
    }
...
impl DifficultyScaler {
    ...
    pub fn adjusted_rate(&self) -> f32 { (self.base_drop_rate * (1.0 + self.difficulty_scale * self.zone_level as f32 * 0.1)).min(1.0) }
}

impl LootTableBuilder {
    pub fn new(id: u32, name: &str) -> Self { Self { table: LootTable::new(id, name) } }
    pub fn with_entry(mut self, item_id: u32, weight: f32) -> Self { self.table.add_entry(item_id, weight, 1, 1); self }
    pub fn build(self) -> LootTable { self.table }
}

impl LootEditor {
    pub fn new() -> Self {
        Self { tables: HashMap::new(), catalog: build_item_catalog(), rng: LootRng::new(42), selected_table: None, show_statistics: false }
    }
...
impl ItemEffect {
    pub fn new(id: u32, name: &str, trigger: EffectTrigger, description: &str, value: f32) -> Self {
        Self { id, name: name.to_string(), trigger, description: description.to_string(), value, duration: 0.0, cooldown: 0.0, proc_chance: 1.0 }
    }
...
impl CraftingSystem {
    ...
    pub fn build_standard_recipes(&mut self) {
        let recipes = vec![
            ...
            CraftingRecipe { id: 4, name: "Steel Sword".into(), inputs: vec![(101,5),(103,2)], output_item_id: 2, output_quantity: 1, skill_required: 30, success_chance: 0.8, byproduct_chance: 0.15, category: "blacksmith".into() },
        ];
...
impl EnchantmentLibrary {
    ...
    pub fn build_standard_library(&mut self) {
        let enchs = vec![
            ...
            Enchantment { id: 2, name: "Fire Aspect".into(), description: "fire".into(), target: EnchantmentTarget::Weapon, effects: vec![EnchantmentEffect::FireDamage(15.0)], rarity: ItemRarity::Uncommon, max_rank: 3, cost: 100 },
            ...
            Enchantment { id: 4, name: "Magic Find".into(), description: "more drops".into(), target: EnchantmentTarget::Accessory, effects: vec![EnchantmentEffect::MagicFind(0.05)], rarity: ItemRarity::Rare, max_rank: 3, cost: 200 },
...
impl EconomySimulator {
    pub fn new(params: EconomySimParams) -> Self {
        ...
        Self { params, money_supply: ms, price_level: 1.0, day: 0, snapshots: Vec::new() }
    }
    pub fn step(&mut self) {
        let new_players = self.params.new_player_spawn_rate_per_day as f64;
        let quit = self.params.quit_rate_per_day as f64;
        let drops = self.params.drops_per_player_per_day as f64 * (self.params.num_players as f64 + new_players - quit);
        let sink = self.params.item_sink_rate as f64 * drops;
        let gold_sink = self.params.gold_sink_rate as f64;
        self.money_supply = self.money_supply * (1.0 - gold_sink) + new_players * 100.0;
        self.price_level *= 1.0 + self.params.inflation_rate_per_day as f64;
        self.snapshots.push(EconomySimSnapshot {
            day: self.day, num_players: self.params.num_players, money_supply: self.money_supply,
            price_level: self.price_level, items_in_circulation: drops as u64, avg_item_value: self.price_level as f32 * 10.0,
        });
        self.day += 1;
    }
    pub fn run(&mut self) {
...
impl PrestigeSystem {
    pub fn new() -> Self {
        Self { current_prestige: 0, total_prestige_points: 0, bonus_magic_find: 0.0, bonus_gold_find: 0.0, bonus_xp: 0.0, prestige_items_unlocked: Vec::new() }
    }
    ...
    pub fn effective_gold_find(&self) -> f32 { self.bonus_gold_find }
}

// ============================================================
// SECTION: ADDITIONAL HELPER IMPLS
// ============================================================

impl DungeonSimulator {
    pub fn new(seed: u64) -> Self { Self { rng: LootRng::new(seed), catalog: build_item_catalog(), difficulty_mult: 1.0 } }
    pub fn simulate_run(&mut self, dungeon: &DungeonRun) -> DungeonLootResult {
        ...
        DungeonLootResult { drops: all_drops, gold: total_gold, bonus_items: Vec::new(), experience: dungeon.total_xp, score: dungeon.rooms.len() as u32 * 100 }
    }
}

impl TreasureChestSystem {
    pub fn new(seed: u64) -> Self { Self { rng: LootRng::new(seed), catalog: build_item_catalog() } }
    pub fn open_chest(&mut self, chest_type: &ChestType, player_level: u32, magic_find: f32) -> ChestOpenResult {
        ...
        for _ in 0..item_count {
            let max_id = self.catalog.len() as u32;
            items.push(self.rng.next_u32_range(1, max_id));
        }
        ChestOpenResult { gold, items, bonus_item: None, experience: gold / 10 }
    }
}

impl VendorNpc {
    pub fn new(id: u32, name: &str, vendor_type: VendorType) -> Self {
        Self { id, name: name.to_string(), vendor_type, items: Vec::new(), restock_timer: 0.0, gold_reserve: 10000, reputation_required: 0 }
    }
    pub fn add_item(&mut self, item: VendorItem) { self.items.push(item); }
    pub fn buy_item(&mut self, item_id: u32, player_gold: &mut u32) -> bool {
        if let Some(idx) = self.items.iter().position(|i| i.item_id == item_id && i.stock > 0) {
            let price = self.items[idx].price;
            if *player_gold >= price {
                *player_gold -= price;
                self.items[idx].stock -= 1;
                self.gold_reserve += price;
                return true;
            }
        }
        false
...
impl Marketplace {
    pub fn new() -> Self { Self { listings: Vec::new(), transactions: Vec::new(), fee_pct: 0.05, next_listing_id: 1 } }
    pub fn list_item(&mut self, seller_id: u32, item_id: u32, price: u32) -> u32 {
        let id = self.next_listing_id;
        self.next_listing_id += 1;
        self.listings.push(MarketListing { id, seller_id, item_id, price, listed_at: 0, expires_at: 7 * 24 });
        id
    }
    pub fn buy_item(&mut self, listing_id: u32, buyer_id: u32) -> Option<u32> {
        if let Some(idx) = self.listings.iter().position(|l| l.id == listing_id) {
            let listing = self.listings.remove(idx);
            let fee = (listing.price as f32 * self.fee_pct) as u32;
            self.transactions.push(MarketTransaction { listing_id, buyer_id, seller_id: listing.seller_id, item_id: listing.item_id, price: listing.price, fee, timestamp: 0 });
            Some(listing.price - fee)
        } else { None }
    }
}
...
impl DropRateAnalyzer {
    ...
    pub fn analyze_table(&mut self, table: &LootTable, catalog: &[Item]) -> DropRateAnalysisReport {
        ...
        let entries = table.entries.iter().map(|e| {
            ...
            EntryAnalysis { item_id: e.item_id, item_name, weight: e.weight, drop_rate: rate, expected_per_100: rate * 100.0, expected_kills_for_drop: if rate > 0.0 { 1.0 / rate } else { f32::INFINITY } }
        }).collect();
        DropRateAnalysisReport { table_id: table.id, table_name: table.name.clone(), entries, total_weight: total, average_drop_rate: if table.entries.is_empty() { 0.0 } else { total / table.entries.len() as f32 } }
    }
}
...
impl LootTableGenerator {
    ...
    pub fn generate_zone_table(&mut self, zone_id: u32, catalog: &[Item]) -> LootTable {
        ...
        for item in catalog.iter().take(20) {
            let weight = item.rarity.base_weight() + self.rng.next_f32() * 5.0;
            table.add_entry(item.id, weight, 1, 1);
        }
        table
    }
    pub fn generate_boss_table(&mut self, boss_id: u32, catalog: &[Item]) -> LootTable {
        let mut table = LootTable::new(boss_id, &format!("Boss {} Loot", boss_id));
        table.boss_exclusive = true;
        for item in catalog.iter().filter(|i| i.rarity.tier() >= 3).take(10) {
            table.add_entry(item.id, item.rarity.base_weight(), 1, 1);
        }
        table
    }
}
...
impl ComprehensiveLootEditor {
    pub fn new() -> Self {
        Self { loot_editor: LootEditor::new(), item_generator: ItemGenerator::new(42), dungeon_simulator: DungeonSimulator::new(43), vendor_manager: VendorManager::new(), marketplace: Marketplace::new(), economy_simulator: None, prestige_system: PrestigeSystem::new(), session_stats: SessionStats::default() }
    }
    pub fn full_simulation(&mut self, runs: u32) {
        for _ in 0..runs {
            self.session_stats.total_rolls += 1;
        }
    }
}
...
impl Default for SessionStats {
    fn default() -> Self {
        Self { total_drops: 0, total_gold_earned: 0, items_crafted: 0, dungeons_completed: 0, total_rolls: 0, legendary_drops: 0, session_start: 0 }
    }
}

impl SeasonalLootManager {
    pub fn new(season: Season) -> Self {
        Self { current_season: season, seasonal_tables: HashMap::new(), bonus_multiplier: 1.0, active: true }
    }
    pub fn add_table(&mut self, table_id: u32, table: LootTable) { self.seasonal_tables.insert(table_id, table); }
    pub fn get_multiplier(&self) -> f32 { if self.active { self.bonus_multiplier } else { 1.0 } }
}
...
impl EventManager {
    pub fn new() -> Self { Self { events: Vec::new(), active_event: None, rng: LootRng::new(99) } }
...
impl ExtendedLootEditor {
    pub fn new() -> Self {
        Self {
            core_editor: LootEditor::new(),
            crafting: CraftingSystem::new(),
            enchantments: EnchantmentLibrary::new(),
            item_generator: ItemGenerator::new(1),
            dungeon_simulator: DungeonSimulator::new(2),
            event_manager: EventManager::new(),
            pity_system: PitySystem::new(0.01, 50, 100),
            streak_counter: 0,
            loot_filter: LootFilterExt::default(),
        }
    }
    pub fn full_roll(&mut self, table_id: u32) -> Vec<DropResult> {
        self.core_editor.roll_table(table_id)
    }
}

impl Default for LootFilterExt {
    fn default() -> Self {
        Self { rules: Vec::new(), enabled: true, log_filtered: false }
    }
}

impl Default for LootContext {
    fn default() -> Self {
        Self { player_level: 1, magic_find: 0.0, gold_find: 0.0, zone_id: 1, kill_streak: 0, is_boss_kill: false, party_size: 1, modifiers: Vec::new() }
    }
}

// ============================================================
// SECTION: LOOT PIPELINE (trait objects)
// ============================================================
...
impl LootModifier for MagicFindModifier {
    fn apply(&self, rolls: &mut Vec<DropResult>, _rng: &mut LootRng) {
        ...
        for i in 0..extra.min(rolls.len()) { rolls[i].count += 1; }
    }
    fn name(&self) -> &str { "MagicFind" }
    fn priority(&self) -> i32 { 10 }
}

pub struct BossKillModifier { pub extra_roll_chance: f32 }
...
impl LootTableHierarchy {
    ...
    pub fn get_inherited_entries(&self, table_id: u32) -> Vec<LootTableEntry> {
        ...
        loop {
            ...
            if let Some(table) = self.tables.get(&current_id) { entries.extend(table.entries.clone()); }
            match self.parent_map.get(&current_id) { Some(&parent) => current_id = parent, None => break }
        }
        entries
    }
}
...
pub struct DropStreakTracker {
    pub dry_streak: u32, pub hot_streak: u32, pub best_streak: u32,
    pub worst_drought: u32, pub total_rolls: u32, pub total_drops: u32, pub last_drop_roll: u32,
}
...
impl DropStreakTracker {
    ...
    pub fn drop_rate(&self) -> f32 {
        if self.total_rolls == 0 { 0.0 } else { self.total_drops as f32 / self.total_rolls as f32 }
    }
    pub fn rolls_since_last_drop(&self) -> u32 { self.total_rolls - self.last_drop_roll }
}

// ============================================================
// SECTION: SALVAGE AND AFFINITY
// ============================================================
...
impl SalvageSystem {
    ...
    pub fn salvage(&mut self, item: &GeneratedItem) -> SalvageResult {
        let bonus = match &item.rarity {
            ItemRarity::Common => 1, ItemRarity::Uncommon => 2, ItemRarity::Rare => 4,
            ItemRarity::Epic => 8, ItemRarity::Legendary => 20, ItemRarity::Mythic => 50, ItemRarity::BossExclusive => 30,
        };
...
#[derive(Clone, Debug)]
pub struct ItemAffinity { pub affinity_id: u32, pub name: &'static str, pub required_items: Vec<u32>, pub bonuses: Vec<AffinityBonus> }

pub struct AffinityRegistry { pub affinities: Vec<ItemAffinity> }
...
impl AffinityRegistry {
    ...
    pub fn total_bonus(&self, equipped: &[u32], stat: &str) -> f32 {
        ...
        self.affinities.iter()
            .filter(|a| a.required_items.iter().all(|id| eq.contains(id)))
...
pub struct LootAchievement {
    ...
    pub target_count: u32, pub current_count: u32, pub completed: bool, pub reward_item_id: Option<u32>,
}
...
impl LootAchievement {
    ...
    pub fn progress_pct(&self) -> f32 { (self.current_count as f32 / self.target_count as f32).min(1.0) }
}

pub struct AchievementTracker { pub achievements: Vec<LootAchievement>, pub completed_ids: Vec<u32> }

impl AchievementTracker {
    pub fn new() -> Self {
        Self {
            achievements: vec![
                LootAchievement::new(1, "First Blood", "First drop", 1, Some(9001)),
                LootAchievement::new(2, "Collector", "100 items", 100, Some(9002)),
                LootAchievement::new(3, "Legendary Hunter", "10 legendaries", 10, Some(9003)),
                LootAchievement::new(4, "Boss Slayer", "100 bosses", 100, Some(9004)),
            ],
            completed_ids: Vec::new(),
        }
    }
    pub fn record(&mut self, amount: u32) -> Vec<u32> {
        let mut done = Vec::new();
        for ach in &mut self.achievements {
            for _ in 0..amount {
                if ach.increment() { done.push(ach.id); }
            }
        }
    ...
    pub fn completion_rate(&self) -> f32 {
        let n = self.achievements.len();
        if n == 0 { return 0.0; }
        self.achievements.iter().filter(|a| a.completed).count() as f32 / n as f32
    }
}
...
#[test]
fn test_alias_table_impl() {
    let table = AliasTable::build(&[1.0, 2.0, 3.0, 4.0]);
...
#[test]
fn test_pity_system_hard() {
    let mut pity = PitySystem::new(0.01, 50, 100);
    let mut rng = LootRng::new(42);
    pity.current_rolls = 99;
    assert!(pity.roll(&mut rng));
}

#[test]
fn test_item_impl() {
    let item = Item::new(1, "Test", ItemType::Weapon, ItemRarity::Common, 10.0);
    assert!(item.market_value() > 0.0);
}

#[test]
fn test_crafting_impl() {
    let mut cs = CraftingSystem::new();
    cs.build_standard_recipes();
    assert!(!cs.recipes.is_empty());
    let mut rng = LootRng::new(42);
    let result = cs.craft(1, 50, &mut rng);
    match result {
        CraftResult::Success { .. } | CraftResult::Failed { .. } => {}
    }
}

#[test]
fn test_drop_streak() {
    let mut tracker = DropStreakTracker::new();
    for _ in 0..10 { tracker.record_roll(false); }
    tracker.record_roll(true);
    assert_eq!(tracker.worst_drought, 10);
    assert!(tracker.drop_rate() > 0.0);
}
...
fn test_loot_rng() {
    ...
    let v = rng.next_f32();
    assert!(v >= 0.0 && v <= 1.0);
    let u = rng.next_u32_range(0, 100);
    assert!(u < 100);
}
