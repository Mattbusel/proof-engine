...
// ============================================================
// PARTICLE MESH (surface topology for particles)
// ============================================================
...
impl ParticleMesh {
    ...
    pub fn build_from_particles(particles: &[ModelParticle], max_edge_len: f32) -> Self {
        ...
        for eid1 in 0..mesh.edges.len() {
            ...
            let a_neighbors: HashSet<usize> = mesh.vert_to_edges.get(&a)
                .map(|eids| eids.iter().map(|&e| { let (ea, eb) = mesh.edges[e]; if ea == a { eb } else { ea } }).collect())
                .unwrap_or_default();
            let b_neighbors: HashSet<usize> = mesh.vert_to_edges.get(&b)
        ...
        for &vi in &self.vertices {
            ...
            if let Some(eids) = self.vert_to_edges.get(&vi) {
                ...
                for &eid in eids {
                    ...
                    sum += positions[if a == vi { b } else { a }];
                    cnt += 1;
                }
...
impl ParticleMaterial {
    ...
    pub fn glyph_for_shade(&self, shade: f32) -> char {
        ...
        let idx = ((shade.clamp(0.0, 1.0) * (self.char_set.len() as f32 - 1.0)).round() as usize)
            .min(self.char_set.len() - 1);
        self.char_set[idx]
    }
...
impl ParticleDelta {
    ...
    pub fn memory_estimate(&self) -> usize {
        self.modified.len() * 28 + self.added.len() * std::mem::size_of::<ModelParticle>()
    }
}
    ...
    pub fn circle(name: impl Into<String>, res: usize) -> Self {
        ...
        for y in 0..res {
            for x in 0..res {
                ...
                s.set_pixel(x, y, smoothstep(0.0, 1.0, (1.0 - (dx * dx + dy * dy).sqrt() / c).clamp(0.0, 1.0)));
            }
        }
        s
    }
...
impl ProceduralPatterns {
    ...
    pub fn stripes(model: &mut ParticleModel, axis: Vec3, width: f32, ca: Vec4, cb: Vec4) {
        ...
        for p in &mut model.particles {
            p.color = if (n.dot(p.position) / width).floor() as i32 % 2 == 0 { ca } else { cb };
        }
    }

    pub fn checkerboard(model: &mut ParticleModel, cell: f32, ca: Vec4, cb: Vec4) {
        for p in &mut model.particles {
            let xi = (p.position.x / cell).floor() as i32;
            let yi = (p.position.y / cell).floor() as i32;
            let zi = (p.position.z / cell).floor() as i32;
    ...
    pub fn radial_gradient(model: &mut ParticleModel, center: Vec3, radius: f32, cc: Vec4, ce: Vec4) {
        for p in &mut model.particles {
            p.color = cc.lerp(ce, ((p.position - center).length() / radius).clamp(0.0, 1.0));
        }
    }
...
pub fn mesh_surface_area(mesh: &ParticleMesh, particles: &[ModelParticle]) -> f32 {
...
pub struct ModelQuality {
    pub particle_count: usize, pub bounding_box_vol: f32, pub density_variance: f32,
    pub avg_neighbor_dist: f32, pub isolated_count: usize, pub cluster_count: usize, pub normal_consistency: f32,
}

impl ModelQuality {
    pub fn analyze(model: &ParticleModel, radius: f32) -> Self {
        let mut q = Self::default(); q.particle_count = model.particles.len(); q.bounding_box_vol = model.bounds.volume();
        ...
        let mut nds = 0.0f32; let mut npc = 0usize;
        for i in 0..model.particles.len() {
            for j in (i+1)..model.particles.len() {
                if (positions[i] - positions[j]).length_squared() <= r2 { nds += model.particles[i].normal.dot(model.particles[j].normal); npc += 1; }
            }
        }
        q.normal_consistency = if npc > 0 { nds / npc as f32 } else { 1.0 };
    ...
    pub fn quantize_colors(model: &mut ParticleModel, palette: &[Vec4]) {
        ...
        for p in &mut model.particles {
            let best = palette.iter()
                .min_by(|a, b| (*a - p.color).length_squared().partial_cmp(&(*b - p.color).length_squared()).unwrap_or(std::cmp::Ordering::Equal))
...
impl LodStreamer {
    ...
    pub fn register_model(&mut self, id: u64, dist: f32) { self.models_by_distance.insert(id, dist); self.active_lods.insert(id, 0); }
...
impl ModelSerializer {
    pub fn serialize_compact(model: &ParticleModel) -> Vec<u8> {
        ...
        for p in &model.particles {
            out.extend_from_slice(&p.position.x.to_le_bytes()); out.extend_from_slice(&p.position.y.to_le_bytes()); out.extend_from_slice(&p.position.z.to_le_bytes());
            out.extend_from_slice(&(p.character as u32).to_le_bytes());
            out.extend_from_slice(&p.color.x.to_le_bytes()); out.extend_from_slice(&p.color.y.to_le_bytes()); out.extend_from_slice(&p.color.z.to_le_bytes()); out.extend_from_slice(&p.color.w.to_le_bytes());
            out.extend_from_slice(&p.emission.to_le_bytes());
            out.extend_from_slice(&p.normal.x.to_le_bytes()); out.extend_from_slice(&p.normal.y.to_le_bytes()); out.extend_from_slice(&p.normal.z.to_le_bytes());
            out.push(p.group_id as u8); out.push(p.layer_id);
        }
        out
    }

    pub fn deserialize_compact(data: &[u8]) -> Option<ParticleModel> {
        if data.len() < 16 || &data[0..4] != b"PMDL" { return None; }
        let mut c = 4usize;
        let id = u64::from_le_bytes(data[c..c+8].try_into().ok()?); c += 8;
        let nl = u32::from_le_bytes(data[c..c+4].try_into().ok()?) as usize; c += 4;
        if c + nl > data.len() { return None; }
        let name = String::from_utf8(data[c..c+nl].to_vec()).ok()?; c += nl;
        let pc = u32::from_le_bytes(data[c..c+4].try_into().ok()?) as usize; c += 4;
        let mut model = ParticleModel::new(id, name);
        for _ in 0..pc {
            let px = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let py = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let pz = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let cc = u32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let cr = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let cg = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let cb = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let ca = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let em = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let nx = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let ny = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let nz = f32::from_le_bytes(data[c..c+4].try_into().ok()?); c += 4;
            let gid = data[c] as u32; c += 1;
            let lid = data[c]; c += 1;
            let ch = char::from_u32(cc).unwrap_or('.');
            let mut part = ModelParticle::new(Vec3::new(px, py, pz), ch, Vec4::new(cr, cg, cb, ca));
            part.emission = em; part.normal = Vec3::new(nx, ny, nz); part.group_id = gid; part.layer_id = lid;
            model.add_particle(part);
        }
        Some(model)
    }
}
...
#[derive(Clone, Debug, Default)]
pub struct ModelClipboard { pub particles: Vec<ModelParticle>, pub pivot: Vec3, pub source_model_id: Option<u64> }

impl ModelClipboard {
    pub fn new() -> Self { Self::default() }

    pub fn copy_selection(&mut self, model: &ParticleModel, sel: &HashSet<usize>) {
        self.particles = sel.iter().filter_map(|&i| model.particles.get(i)).cloned().collect();
        self.pivot = if self.particles.is_empty() { Vec3::ZERO } else {
            self.particles.iter().map(|p| p.position).fold(Vec3::ZERO, |a, b| a + b) / self.particles.len() as f32
        };
    ...
    pub fn paste_at(&self, model: &mut ParticleModel, target: Vec3) {
        ...
        model.add_particles_bulk(self.particles.iter().map(|p| { let mut np = p.clone(); np.position += offset; np }).collect());
    }
...
#[derive(Clone, Debug)]
pub struct CatmullRomSpline { pub control_points: Vec<Vec3>, pub alpha: f32 }

impl CatmullRomSpline {
    pub fn new(alpha: f32) -> Self { Self { control_points: Vec::new(), alpha } }
    pub fn add_point(&mut self, p: Vec3) { self.control_points.push(p); }

    fn seg_t(p0: Vec3, p1: Vec3, alpha: f32) -> f32 { (p1 - p0).length().powf(alpha) }

    pub fn evaluate(&self, t: f32) -> Vec3 {
        let n = self.control_points.len();
        if n < 2 { return self.control_points.first().copied().unwrap_or(Vec3::ZERO); }
        let max_t = (n - 1) as f32; let t = t.clamp(0.0, max_t);
        let seg = (t as usize).min(n - 2); let lt = t - seg as f32;
        let p0 = self.control_points[seg.saturating_sub(1)];
        let p1 = self.control_points[seg];
        let p2 = self.control_points[(seg + 1).min(n - 1)];
        let p3 = self.control_points[(seg + 2).min(n - 1)];
        let t0 = 0.0f32; let t1 = t0 + Self::seg_t(p0, p1, self.alpha);
        let t2 = t1 + Self::seg_t(p1, p2, self.alpha); let t3 = t2 + Self::seg_t(p2, p3, self.alpha);
        let tp = t1 + lt * (t2 - t1);
        let sd = |a: Vec3, b: f32| -> Vec3 { if b.abs() < EPSILON { Vec3::ZERO } else { a / b } };
        let a1 = sd(p0 * (t1 - tp) + p1 * (tp - t0), t1 - t0);
        let a2 = sd(p1 * (t2 - tp) + p2 * (tp - t1), t2 - t1);
        let a3 = sd(p2 * (t3 - tp) + p3 * (tp - t2), t3 - t2);
        let b1 = sd(a1 * (t2 - tp) + a2 * (tp - t0), t2 - t0);
        let b2 = sd(a2 * (t3 - tp) + a3 * (tp - t1), t3 - t1);
        sd(b1 * (t2 - tp) + b2 * (tp - t1), t2 - t1)
    }
    ...
    pub fn sample(&self, n: usize) -> Vec<Vec3> {
        ...
        (0..n).map(|i| self.evaluate(i as f32 / (n as f32 - 1.0).max(1.0) * max_t)).collect()
    }

    pub fn deform_along_path(&self, model: &mut ParticleModel, indices: &HashSet<usize>, axis: Vec3) {
        let n = axis.normalize();
        let ps: Vec<Vec3> = indices.iter().filter_map(|&i| model.particles.get(i)).map(|p| p.position).collect();
        if ps.is_empty() { return; }
        let min_t = ps.iter().map(|&p| n.dot(p)).fold(f32::MAX, f32::min);
        let max_t = ps.iter().map(|&p| n.dot(p)).fold(f32::MIN, f32::max);
        let range = (max_t - min_t).max(EPSILON);
        let slen = (self.control_points.len().max(1) - 1) as f32;
        for &i in indices {
            if let Some(p) = model.particles.get_mut(i) {
                if p.locked { continue; }
                let t = (n.dot(p.position) - min_t) / range * slen;
                let sp = self.evaluate(t);
                let perp = p.position - n * n.dot(p.position);
                p.position = sp + perp;
            }
        }
        model.recompute_bounds();
...
pub struct BrushPreset {
    pub name: String, pub kind: BrushKind, pub radius: f32, pub strength: f32,
    pub density: f32, pub falloff: FalloffCurve, pub character: char, pub color: Vec4,
}

impl BrushPreset {
    pub fn new(name: impl Into<String>, kind: BrushKind) -> Self {
        Self { name: name.into(), kind, radius: 1.0, strength: 0.5, density: 4.0,
               falloff: FalloffCurve::Smooth, character: '.', color: Vec4::ONE }
    }

    pub fn to_params(&self) -> BrushParams {
        BrushParams { kind: self.kind.clone(), radius: self.radius, strength: self.strength,
                      density: self.density, color: self.color, character: self.character, falloff: self.falloff.clone() }
    }
}

#[derive(Clone, Debug, Default)]
pub struct BrushPresetLibrary { pub presets: BTreeMap<String, BrushPreset> }

impl BrushPresetLibrary {
    pub fn new() -> Self { let mut l = Self::default(); l.add_defaults(); l }

    fn add_defaults(&mut self) {
        let mut add = BrushPreset::new("Default Add", BrushKind::Add); add.density = 8.0;
        self.presets.insert(add.name.clone(), add);
        let mut sm = BrushPreset::new("Heavy Smooth", BrushKind::Smooth); sm.strength = 0.8; sm.radius = 2.0;
        self.presets.insert(sm.name.clone(), sm);
        let inf = BrushPreset::new("Inflate", BrushKind::Inflate); self.presets.insert(inf.name.clone(), inf);
        let p = BrushPreset::new("Pinch", BrushKind::Pinch); self.presets.insert(p.name.clone(), p);
        let f = BrushPreset::new("Flatten", BrushKind::Flatten); self.presets.insert(f.name.clone(), f);
    }

    pub fn add(&mut self, p: BrushPreset) { self.presets.insert(p.name.clone(), p); }
    pub fn get(&self, name: &str) -> Option<&BrushPreset> { self.presets.get(name) }
    pub fn names(&self) -> Vec<&str> { self.presets.keys().map(|s| s.as_str()).collect() }
}
...
impl ModelRenderData {
    pub fn build(editor: &ModelEditor, _camera_pos: Vec3, camera_dist: f32, lod_bias: f32) -> Self {
        ...
        if model.lod_levels.is_empty() {
            for p in &model.particles { data.glyphs.push((p.position, p.character, p.color, p.emission)); }
        } else {
            let li = lod_idx.min(model.lod_levels.len() - 1);
            for &pi in &model.lod_levels[li].particles {
                if let Some(p) = model.particles.get(pi) { data.glyphs.push((p.position, p.character, p.color, p.emission)); }
            }
        }
        if editor.wireframe_mode {
            for (a, b) in VisualizationHelper::find_edges(&model.particles, editor.brush_radius * 0.5) {
        ...
        if editor.normal_vis {
            for p in &model.particles { data.normal_tips.push((p.position + p.normal * 0.2, '^', Vec4::new(0.0, 1.0, 0.5, 1.0))); }
        }
        if editor.show_bounds {
            for p in GridHelper::bbox_wireframe(&model.bounds, 8, '+', Vec4::new(1.0, 1.0, 0.0, 0.8)) {
...
impl ModelingToolContext2 {
    pub fn new() -> Self {
        Self {
            editor: ModelEditor::new(), clipboard: ModelClipboard::new(),
            presets: BrushPresetLibrary::new(), spline: CatmullRomSpline::new(0.5),
            material_lib: MaterialLibrary::new(), group_mgr: GroupManager::new(),
            animations: HashMap::new(),
        }
    }

    pub fn apply_preset(&mut self, name: &str, hit: Vec3, ray: Ray3) {
        ...
        self.editor.apply_brush(ray, hit);
    }

    pub fn copy(&mut self) {
        let sel = self.editor.selection.clone();
        if let Some(m) = self.editor.active_model() { self.clipboard.copy_selection(m, &sel); }
    }

    pub fn paste(&mut self, target: Vec3) {
        if self.clipboard.is_empty() { return; }
        let cb = self.clipboard.clone();
        if let Some(m) = self.editor.active_model_mut() { cb.paste_at(m, target); }
    }
...
mod integration_tests {
    ...
    #[test]
    fn test_catmull_sample() {
        let mut sp = CatmullRomSpline::new(0.5);
        sp.add_point(Vec3::ZERO); sp.add_point(Vec3::X); sp.add_point(Vec3::new(2.0, 1.0, 0.0)); sp.add_point(Vec3::new(3.0, 0.0, 0.0));
        let pts = sp.sample(10);
    ...
    fn test_serialization_roundtrip() {
        let mut model = ParticleModel::new(42, "serial");
        model.add_particle(ModelParticle::new(Vec3::new(1.0, 2.0, 3.0), 'Z', Vec4::new(0.5, 0.3, 0.1, 1.0)));
        let bytes = ModelSerializer::serialize_compact(&model);
        let recovered = ModelSerializer::deserialize_compact(&bytes).unwrap();
        assert_eq!(recovered.particles.len(), 1);
        assert_eq!(recovered.particles[0].character, 'Z');
        assert!((recovered.particles[0].position - Vec3::new(1.0, 2.0, 3.0)).length() < 1e-4);
    }

    #[test]
    fn test_modeling_context2() {
        ...
        ctx.copy();
        assert_eq!(ctx.clipboard.count(), 40);
        ctx.paste(Vec3::new(3.0, 0.0, 0.0));
