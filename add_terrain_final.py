...
impl VerticalCurve {
    ...
    pub fn elevation_at_station(&self, station_m: f32) -> f32 {
        ...
        let x = (station_m - bvc).max(0.0).min(self.length_m);
...
impl GradeOptimizer {
    ...
    pub fn optimize(&mut self, profile: &[(f32, f32)], _spacing: f32) -> Vec<GradeSegment> {
        let segs: Vec<GradeSegment> = profile.windows(2).map(|w| GradeSegment { start_station: w[0].0, end_station: w[1].0, start_distance: w[0].0, end_distance: w[1].0, grade_percent: if w[1].0 > w[0].0 { (w[1].1 - w[0].1) / (w[1].0 - w[0].0) * 100.0 } else { 0.0 }, is_steep: false, grade: 0.0, cut_volume: 0.0, fill_volume: 0.0 }).collect();
...
impl IntersectionCapacityAnalysis {
    ...
    pub fn add_approach(&mut self, volume: f32, _phf: f32, _turn_type: TurnType) { self.approaches.push(ApproachMovement { volume_vph: volume, phf: 0.9, turn_type: TurnType::Through, shared_lane: false }); }
    pub fn add_phase(&mut self, _approach_ids: Vec<usize>, green_time: f32) { self.phases.push(SignalPhase { movements: Vec::new(), green_time, yellow_time: 3.0, all_red_time: 1.0 }); }
...
impl RoadNoiseMonitor {
    pub fn new(monitor_id: u32, distance_m: f32, l_eq: f32, l_90: f32) -> Self { Self { monitor_id, distance_from_road_m: distance_m, l_eq_dba: l_eq, l_10_dba: l_eq + 5.0, l_90_dba: l_90, peak_hour_db: l_eq + 3.0, fhwa_noise_abatement_criteria: 67.0 } }
